...
def layerNINParams(shape, hiddenExpand, outExpand):
    ...
    w2 = utils.weightInitNIN2_3(shape[:2] + (hiddenExpand, shape[2]), 'w2')
    w3 = utils.weightInitColfc((shape[0], outExpand, shape[1] * hiddenExpand), 'w3')
    b1 = utils.biasInit(shape[:3], 'b1')
    b2 = utils.biasInit(shape[:2] + (hiddenExpand,), 'b2')
    b3 = utils.biasInit((shape[0], outExpand), 'b3')
