...
__author__ = 'zfh'
'''
32滤波器产生32*16个特征图，16个按行全连接为2个特征图，32*2个按列全连接产生下一层的2个特征图
顶层使用全局平均池化层
'''
import time
...
#             relu
#             (n,16,r,c)**(3,16,1,1)=(n,3,r,c)
#             relu
#         concatenate(32*(n,3,r,c), axis=1)
#         (n,32*3,r,c)**(2,32*3,1,1)=(n,2,r,c)
#         relu
...
def nin(X, param):
    ...
    w2 = w2.dimshuffle(0, 1, 2, 3, 'x', 'x')  # (64,32,3,16,1,1)
    w3 = w3.dimshuffle(0, 1, 2, 'x', 'x')  # (64,2,32*3,1,1)
    b1 = b1.dimshuffle(0, 1, 'x', 2, 'x', 'x')  # (64,32,1,16,1,1)
    b2 = b2.dimshuffle(0, 1, 'x', 2, 'x', 'x')  # (64,32,1,3,1,1)
    ...
    results, updates = scan(fn=metaOp1,
                            ...
                            strict=True)  # (64*32,n,3,r,c)
    metaShape1 = results.shape[-4], results.shape[-3], results.shape[-2], results.shape[-1]
    reshaped1 = results.reshape((w1.shape[0], w1.shape[1]) + metaShape1)  # (64,32,n,3,r,c)
    permuted1 = T.transpose(reshaped1, axes=(0, 2, 1, 3, 4, 5))  # (64,n,32,3,r,c)
    metaShape2 = permuted1.shape[-2], permuted1.shape[-1]
    reshaped2 = permuted1.reshape((permuted1.shape[0], permuted1.shape[1], -1) + metaShape2)  # (64,n,32*3,r,c)
    indexi = T.arange(w1.shape[0], dtype='int32')  # (0:64)
    results, updates = scan(fn=metaOp2,
                            sequences=[indexi],
                            outputs_info=None,
                            non_sequences=[reshaped2, w3, b3],
                            strict=True)  # (64,n,2,r,c)
    permuted2 = T.transpose(results, axes=(1, 0, 2, 3, 4))  # (n,64,2,r,c)
    metaShape3 = permuted2.shape[-2], permuted2.shape[-1]
    reshaped3 = permuted2.reshape((permuted2.shape[0], -1) + metaShape3)  # (n,64*2,r,c)
    return reshaped3
...
def layerNINParams(shape, hiddenExpand, outExpand):
    w1 = initial.weightInitNIN1_3(shape, 'w1')
    w2 = initial.weightInitNIN2_3(shape[:2] + (hiddenExpand, shape[2]), 'w2')
    w3 = initial.weightInitColfc((shape[0], outExpand, shape[1] * hiddenExpand), 'w3')
    b1 = initial.biasInit(shape[:3], 'b1')
    b2 = initial.biasInit(shape[:2] + (hiddenExpand,), 'b2')
    b3 = initial.biasInit((shape[0], outExpand), 'b3')
...
class CConvNet(object):
    def __init__(self, fin, f1, nin1, f2, nin2, f3, nin3, hiddenExpand, outExpand, h1, outputs,
        ...
        self.paramsNIN.append(layerNINParams((f1, fin, nin1, 3, 3), hiddenExpand, outExpand))
        self.paramsNIN.append(layerNINParams((f2, f1 * outExpand, nin2, 3, 3), hiddenExpand, outExpand))
        self.paramsNIN.append(layerNINParams((f3, f2 * outExpand, nin3, 3, 3), hiddenExpand, outExpand))
        # 全局平均池化层
        self.paramsConv.append(layerConvParams((h1, f3 * outExpand, 1, 1)))
...
def main():
    ...
    f1, nin1, f2, nin2, f3, nin3, hiddenExpand, outExpand, h1 = 16, 5, 32, 3, 64, 2, 3, 2, 64
    params = basicUtils.randomSearch(nIter=10)
    cvErrorList = []
    for param, num in zip(params, range(len(params))):
        lr, C = param
        print '*' * 40, num, 'parameters', param, '*' * 40
        convNet = CConvNet(3, f1, nin1, f2, nin2, f3, nin3, hiddenExpand, outExpand, h1, 10, lr, C, 0.2, 0.5)
