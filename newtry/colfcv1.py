...
def layerNINParams(shape, expand):
    w1 = initial.weightInitCNN3(shape, 'w1')
    w2 = initial.weightInitColfc((shape[0], expand, shape[1]), 'w2')
    b1 = initial.biasInit(shape[:2], 'b1')
    b2 = initial.biasInit((shape[0], expand), 'b2')
    return [w1, w2, b1, b2]
...
class CConvNet(object):
    def __init__(self, fin, f1, f2, f3, expand, h1, outputs,
        ...
        self.paramsNIN.append(layerNINParams((f1, fin, 3, 3), expand))
        self.paramsNIN.append(layerNINParams((f2, f1 * expand, 3, 3), expand))
        self.paramsNIN.append(layerNINParams((f3, f2 * expand, 3, 3), expand))
        # 全局平均池化层
